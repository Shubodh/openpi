...
echo ""
echo "Set permissions (run once — see docs/runpod_setup.md §0 Step 3 for details):"
echo "  mkdir -p ~/.claude && cat > ~/.claude/settings.json << 'EOF'"
echo '  { "permissions": { "allow": ["Bash","Read","Edit","Write","Glob","Grep"] }, "defaultMode": "bypassPermissions" }'
echo "  EOF"
echo "  mkdir -p ~/.codex && echo 'approval_policy = \"never\"' > ~/.codex/config.toml"
echo ""
