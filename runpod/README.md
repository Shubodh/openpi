...
| `setup_pod.sh` | Every pod restart (~1-2 min) | Reinstalls uv + Claude Code + Codex (wiped on stop); restores env vars |
| `setup_agents.sh` | Once per volume | Installs Node.js + Claude Code + Codex CLI; prints API key and permissions setup instructions |
