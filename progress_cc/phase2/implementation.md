...
C1-lang failed 0/5 — language-only patching insufficient for cross-pair flip (consistent with Phase 1 and Phase 2a findings). C1-img also failed 0/5, so the Phase 2a image region `294-587` is not sufficient for the harder both-object-and-destination Pair A flip. C1-full passed 5/5, so cross-pair patching is possible via broader prefix KV. Binary half 0–393 failed 0/10; binary half 394–787 recovered 3/10. Its first child, 394–590, failed 0/10; 591–787 is next. All C2 and Pair D rows pending.
...
**Last updated:** 2026-05-06 — C1 binary child 394–590 failed 0/10; running 591–787 next.
...
**Next action:** Run C1 binary child positions 591–787, N=10. Promote/recurse if meaningful; if it fails, 394–787 remains the smallest defensible C1 set.
